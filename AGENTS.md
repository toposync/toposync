...
- When adding frontend routes, links, API calls, event streams, WebSockets, or extension asset/file URLs, keep Home Assistant ingress paths in mind. Browser-visible absolute paths must go through the TopoSync base path helpers (`resolveToposyncUrl`, router helpers, or equivalent) so `/api/hassio_ingress/...` prefixes are preserved, and route matching should use the logical app path after stripping `window.__TOPOSYNC_PUBLIC_BASE_PATH__`.
