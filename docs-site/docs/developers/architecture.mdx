...
- PTZ Attention: lifecycle-aware arbitration that selects relevant semantic events, focuses allowed calibrated PTZ presets, and coordinates return behavior.
...
The Cameras extension owns physical PTZ actuation, leases, command fencing, and manual takeover. PTZ Attention owns semantic selection, focus sessions, timing, and return policy. This keeps device control generic while allowing event-specific attention behavior to evolve independently.
