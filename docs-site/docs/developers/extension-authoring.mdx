...
description: "Author Toposync extensions as Python wheels with entry points, extension manifests, static assets, and frontend remotes."
keywords: ["toposync", "local-first", "home automation", "extensions", "entry points", "plugin authoring"]
...
For settings that control domain resources, override the synchronous
`settings_authorization_requirements()` hook. It receives the effective current and
proposed extension settings and returns every additional scoped permission that must
be authorized before either `PATCH /api/settings/extensions/<id>` or
`PUT /api/settings` can persist the change.
...
Return only resources whose effective records changed. If the affected resource
cannot be determined safely, return a wildcard selector (`"*"`). The hook must be
synchronous and return complete `action`, `resource_type`, and `resource_selector`
strings; exceptions or malformed results fail closed and settings are not persisted.
The contract remains active for an installed extension even when its runtime is
disabled, preventing protected settings from being staged for a later reactivation.
...
Compatibility is enforced before `setup()`. If the core version or required extension versions do not match, the extension is skipped instead of partially loaded. Required extensions are set up before their dependents; shutdown callbacks registered during `setup()` consequently run in reverse dependency order.
...
## Stabilization path {/* #stabilization-path */}
