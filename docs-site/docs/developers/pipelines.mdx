...
- `toposync-ext-ptz-attention` registers the origin-only `ptz_attention.request` operator that turns complete semantic event lifecycles into PTZ attention intents;
