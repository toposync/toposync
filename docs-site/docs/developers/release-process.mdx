...
description: "Follow the Toposync release process for Python packages, first-party extensions, Docker builds, and the Home Assistant add-on."
keywords: ["toposync", "local-first", "home automation", "release process", "PyPI", "Home Assistant add-on"]
...
| First-party extensions | `toposync/toposync` | `toposync-ext-structural`, `toposync-ext-models`, `toposync-ext-home-assistant`, `toposync-ext-images`, `toposync-ext-cameras`, `toposync-ext-cinematic`, `toposync-ext-vision`, `toposync-ext-ptz-attention`, `toposync-ext-streaming`, `toposync-ext-ai`, `toposync-ext-spatial-video` | Move together as one extension bundle version unless a deliberate split is documented |
...
### 10. Publish Docker images to GHCR {/* #publish-docker-images-to-ghcr */}

Only publish Docker images after production PyPI validation succeeds. The public
image build installs from PyPI, not from the local checkout.

Push the release tag or run the Docker publish workflow manually from the exact
release ref:
...
- `ghcr.io/toposync/toposync:X.Y.Z`, `:X.Y`, and `:latest` for CPU;
- `ghcr.io/toposync/toposync:X.Y.Z-cuda`, `:X.Y-cuda`, and `:cuda` for CUDA.

The CPU image is multi-arch for `linux/amd64` and `linux/arm64`. The CUDA image
is `linux/amd64` and requires a Linux host with NVIDIA driver and NVIDIA
Container Toolkit.

After the first GHCR publish, confirm the package is public. If it was created
as private, change package visibility in the GitHub organization package
settings, then verify anonymous pull:
...
Run the public image smoke test:
...
On a real NVIDIA host, also run `nvidia-smi` inside the CUDA container.

### 11. Release and tag the Home Assistant add-on {/* #release-and-tag-the-home-assistant-add-on */}
...
### 12. Publish release notes {/* #publish-release-notes */}
...
- CI publishes CPU and CUDA images to GHCR from the release tag after PyPI is visible;
