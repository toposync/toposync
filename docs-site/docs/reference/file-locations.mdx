...
description: "Find Toposync file locations for data directories, add-on storage, models, logs, and local development files."
keywords: ["toposync", "local-first", "home automation", "file locations", "data directory", "storage"]
...
| `ptz_attention/attention.sqlite3` | PTZ Attention profiles, decisions, and focus-session history. SQLite may keep `-wal` and `-shm` companion files beside it while Toposync is running. |
| `telemetry/*.tlm1` | Pipeline telemetry snapshots. |
| `runtime/` | Runtime-generated files and extension runtime state. |
| `runtime/cameras/ptz-control.json` | Persisted PTZ fencing counters used to reject stale camera-control commands after a restart. |
