...
description: "Crie extensões Toposync como wheels Python com entry points, manifests, assets estáticos e frontend remotes."
keywords: ["toposync", "local-first", "automação residencial", "extensões", "entry points", "plugin authoring"]
...
Para configurações que controlam recursos do domínio, sobrescreva o hook síncrono
`settings_authorization_requirements()`. Ele recebe as configurações efetivas atual e
proposta da extensão e retorna cada permissão escopada adicional que precisa ser
autorizada antes de `PATCH /api/settings/extensions/<id>` ou `PUT /api/settings`
persistir a mudança.

```py
def settings_authorization_requirements(
    self, *, current_settings: object, proposed_settings: object
) -> list[dict[str, str]]:
    return [{
        "action": "example:device:configure",
        "resource_type": "example:device",
        "resource_selector": "device-1",
    }]
```

Retorne somente os recursos cujos registros efetivos mudaram. Se não for possível
determinar o recurso afetado com segurança, retorne o seletor global (`"*"`). O hook
deve ser síncrono e fornecer strings completas em `action`, `resource_type` e
`resource_selector`; exceções ou resultados malformados falham de forma fechada e as
configurações não são persistidas. O contrato continua ativo para uma extensão
instalada mesmo quando seu runtime está desabilitado, impedindo que configurações
protegidas sejam preparadas para uma reativação posterior.
...
Compatibilidade é aplicada antes de `setup()`. Se a versão do core ou das extensões exigidas não bater, a extensão é ignorada em vez de ser carregada parcialmente. Extensões exigidas executam `setup()` antes de suas dependentes; por consequência, callbacks de desligamento registrados no `setup()` rodam na ordem inversa das dependências.
...
## Caminho de estabilização {/* #stabilization-path */}
