---
title: Pipelines
description: "Aprenda como pipelines Toposync produzem Spatial Events por meio de grafos de operadores, execução local, processamento remoto e contratos de extensões."
keywords: ["toposync", "local-first", "Spatial Events", "Spatial Intelligence", "automação residencial", "pipelines", "operadores", "processing server"]
...
Na camada de produto, pipelines são o caminho principal para transformar entrada de câmera e estado de automação em **Spatial Events** e em futuros recursos de **Spatial Intelligence**. Na camada de runtime, esses conceitos continuam usando contratos concretos de packet, payload, lifecycle, artifact e operador.
...
Para eventos de produto, a identidade canônica é `payload.subject`. **Spatial
Events** são eventos de produto com contexto mapeado suficiente para UI,
armazenamento, notificações e integrações raciocinarem sobre onde algo
aconteceu. Pacotes de evento de câmera e visão devem definir:
...
- `toposync-ext-ptz-attention` registra o operador `ptz_attention.request`, restrito ao servidor principal, que transforma ciclos de vida completos de eventos semânticos em intenções de atenção PTZ;
