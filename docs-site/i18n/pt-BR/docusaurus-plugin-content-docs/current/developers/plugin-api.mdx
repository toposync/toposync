---
title: Plugin API
description: "Use o contrato público da Toposync plugin API para frontend remotes de extensões, navegação, settings e integração de UI."
keywords: ["toposync", "local-first", "automação residencial", "plugin API", "extensões frontend", "module federation"]
...
Para dados específicos da extensão, exponha suas próprias rotas backend na extensão Python e chame essas rotas com `host.api.fetch()` ou os helpers de `@toposync/plugin-api`, como `requestJson()`.
