...
description: "Siga o processo de release do Toposync para pacotes Python, extensões first-party, builds Docker e add-on Home Assistant."
keywords: ["toposync", "local-first", "automação residencial", "processo de release", "PyPI", "add-on Home Assistant"]
...
| Extensões first-party | `toposync/toposync` | `toposync-ext-structural`, `toposync-ext-models`, `toposync-ext-home-assistant`, `toposync-ext-images`, `toposync-ext-cameras`, `toposync-ext-cinematic`, `toposync-ext-vision`, `toposync-ext-ptz-attention`, `toposync-ext-streaming`, `toposync-ext-ai`, `toposync-ext-spatial-video` | Evoluem juntas como uma versão do bundle de extensões, a menos que uma separação deliberada seja documentada |
...
git add docs-site/docs docs-site/i18n/pt-BR/docusaurus-plugin-content-docs/current scripts/install_windows_processing_server.ps1
...
for project in \
  ...
  extensions/images \
  extensions/cameras \
  extensions/cinematic \
  extensions/vision \
  extensions/ptz_attention \
...
uv pip install \
  ...
  toposync-ext-cinematic==E.F.G \
  toposync-ext-vision==E.F.G \
  toposync-ext-ptz-attention==E.F.G \
...
### 10. Publicar imagens Docker no GHCR {/* #publish-docker-images-to-ghcr */}

Publique imagens Docker somente depois que a validação no PyPI de produção
passar. O build público da imagem instala a partir do PyPI, não do checkout
local.

Envie a tag de release ou rode manualmente o workflow de publicação Docker a
partir da ref exata da release:
...
- `ghcr.io/toposync/toposync:X.Y.Z`, `:X.Y` e `:latest` para CPU;
- `ghcr.io/toposync/toposync:X.Y.Z-cuda`, `:X.Y-cuda` e `:cuda` para CUDA.

A imagem CPU é multi-arch para `linux/amd64` e `linux/arm64`. A imagem CUDA é
`linux/amd64` e exige host Linux com driver NVIDIA e NVIDIA Container Toolkit.

Depois do primeiro publish no GHCR, confirme que o pacote está público. Se ele
nascer privado, altere a visibilidade nas configurações do package na
organização GitHub, então valide pull anônimo:

```bash
docker logout ghcr.io || true
docker pull ghcr.io/toposync/toposync:X.Y.Z
docker pull ghcr.io/toposync/toposync:X.Y.Z-cuda
```

Rode o smoke da imagem pública:

```bash
python scripts/check_docker_registry_image.py --image ghcr.io/toposync/toposync:X.Y.Z --platform linux/amd64
python scripts/check_docker_registry_image.py --image ghcr.io/toposync/toposync:X.Y.Z --platform linux/arm64
python scripts/check_docker_registry_image.py --image ghcr.io/toposync/toposync:X.Y.Z-cuda --platform linux/amd64 --expect-cuda
```

Em um host NVIDIA real, rode também `nvidia-smi` dentro do container CUDA.

### 11. Publicar e taguear o add-on Home Assistant {/* #release-and-tag-the-home-assistant-add-on */}
...
### 12. Publicar release notes {/* #publish-release-notes */}
...
- CI publica imagens CPU e CUDA no GHCR a partir da tag de release depois que o PyPI estiver visível;
