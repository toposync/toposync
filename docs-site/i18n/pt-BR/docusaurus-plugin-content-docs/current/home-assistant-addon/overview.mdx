...
description: "Visão geral do add-on Toposync para Home Assistant OS, incluindo ingress, acesso direto, câmeras e integração local."
keywords: ["toposync", "local-first", "automação residencial", "Home Assistant OS", "add-on", "visão geral"]
...
Entidades nativas de câmera do Home Assistant são um contrato de reprodução
...
Para reprodução web e móvel normal, comece por `18756/tcp`. RTSP e WebRTC devem
ser mapeados somente quando você realmente precisar desses protocolos.
