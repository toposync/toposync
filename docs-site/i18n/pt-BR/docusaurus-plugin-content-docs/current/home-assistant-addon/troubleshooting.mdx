...
Para reprodução web/móvel fora da sidebar, comece com HLS proxificado por:
...
Verifique se as URLs de reprodução apontam para o host e porta direta do
Toposync, por exemplo:
...
O player da sidebar do Toposync e a reprodução de câmera pelo Home Assistant Cloud
...
Depois garanta que o cliente usa um hostname ou IP que o add-on consegue
anunciar como host WebRTC. Se isso estiver instável, mude a reprodução do
dashboard para `Auto` ou `HLS`, a menos que baixa latência seja necessária.

Se HLS ou MSE está ao vivo e apenas WebRTC mostra aviso, trate o aviso como
diagnóstico de baixa latência. Ele não é a falha principal de reprodução, a menos
