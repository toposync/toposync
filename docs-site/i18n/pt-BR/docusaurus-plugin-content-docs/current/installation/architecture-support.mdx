...
| Docker CPU | Suportado | Imagem pública GHCR para `amd64` e `arm64` |
| Docker CUDA | Linux + NVIDIA | Imagem pública GHCR para `amd64`; exige driver NVIDIA e NVIDIA Container Toolkit |
...
| Processing server Docker | Suportado | Imagem pública CPU; imagem pública CUDA em Linux `amd64` com NVIDIA |
