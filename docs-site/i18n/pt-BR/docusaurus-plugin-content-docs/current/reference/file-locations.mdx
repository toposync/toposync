...
description: "Encontre locais de arquivos do Toposync para diretórios de dados, add-on, modelos, logs e desenvolvimento local."
keywords: ["toposync", "local-first", "automação residencial", "locais de arquivos", "diretório de dados", "armazenamento"]
...
| `ptz_attention/attention.sqlite3` | Perfis, decisões e histórico de sessões de foco da Atenção PTZ. O SQLite pode manter arquivos auxiliares `-wal` e `-shm` ao lado enquanto o Toposync estiver em execução. |
| `telemetry/*.tlm1` | Snapshots de telemetria de pipelines. |
| `runtime/` | Arquivos gerados em runtime e estado de runtime de extensões. |
| `runtime/cameras/ptz-control.json` | Contadores persistentes de proteção PTZ usados para rejeitar comandos antigos de controle da câmera depois de uma reinicialização. |
