...
export function FloatingVideoPanel({ title, children, hidden = false, initialLeft = 16, initialTop = 70, initialWidth = 320, minimizable = false, onMinimizedChange }: {
    ...
    function maximumWidth() { return Math.max(120, Math.min(bounds.current.width - 16, (bounds.current.height - 142) * 16 / 9)); }
    function constrain(value: typeof box) { const maximum = maximumWidth(); const width = Math.min(Math.max(Math.min(220, maximum), value.width), maximum); return { width, left: Math.max(8, Math.min(value.left, bounds.current.width - width - 8)), top: Math.max(54, Math.min(value.top, bounds.current.height - (minimized ? 36 : width * 9 / 16 + 36) - 52)) }; }
    ...
    function toggleMinimized() { const next = !minimized; setMinimized(next); onMinimizedChange?.(next); }
    ...
    {!minimized && <div className="livePanoramaResizeHandle" role="slider" tabIndex={0} aria-label={`Redimensionar ${title}`} aria-valuenow={Math.round(box.width)} aria-valuemin={Math.min(220, maximumWidth())} aria-valuemax={maximumWidth()} onKeyDown={event => { if (['ArrowLeft', 'ArrowRight'].includes(event.key)) {
        event.preventDefault();
