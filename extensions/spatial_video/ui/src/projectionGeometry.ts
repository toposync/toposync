...
import type { CameraControlPointSet, MediaContentRect, WorldPoint } from "./types";
...
export function createProjectionGeometry(
  ...
  options?: { clipPolygon?: WorldPoint[] | null; uvRect?: MediaContentRect | null },
): THREE.BufferGeometry | null {
  const meshData = projectionStrategies[strategyId].buildMesh(set, {
    gridDivisions: meshDensity,
    clipPolygon: options?.clipPolygon ?? null,
    uvRect: options?.uvRect ?? null,
  });
