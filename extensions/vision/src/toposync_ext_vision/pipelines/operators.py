...
def register_vision_pipeline_operators(registry: OperatorRegistry) -> None:
    ...
    if registry.get("vision.group_events") is None:
        registry.register_operator(
            operator_id="vision.group_events",
            description=(
                "Grouped event lifecycle. Consumes individual event packets from vision.track "
                "and emits aggregated group_event subjects for quieter storage, throttling, "
                "and notifications."
            ),
    ...
    if registry.get("vision.spatial_relation_event") is None:
        registry.register_operator(
            ...
            capabilities=["vision", "tracking", "grouping", "relation", "event", "split_stream"],
