...
function IndividualIdentity({ notification, i18n, summary = false, selectedOccurrence }: { notification: Notification; i18n: HostI18n; summary?: boolean; selectedOccurrence?: string }) {
  ...
  return <section className="identityGallery" aria-label={t("title")}><style>{styles}</style>
    ...
    {editing && writable ? gallery.error ? <ErrorMessage t={t} /> : !gallery.data ? <p role="status">{t("loading")}</p> : <Editor species={detail.species} observations={detail.observations} identities={gallery.data.identities} revision={detail.revision} t={t} onClose={close} initialIdentity={current?.id} /> : <>
      {candidates.length > 0 && <div><p className="identityMuted">{t("suggestions")}</p>{candidates.map((item) => <div className="identityToolbar" key={item.id}><span>{item.name}</span><button type="button" className="chipButton" disabled={busy || !writable} onClick={(event) => { const trigger = event.currentTarget; void run("/curation", { action: "reject", expected_revision: detail.revision, values: { identity_id: item.id, observation_ids: detail.observations.map((sample) => sample.id) } }).then((saved) => { requestAnimationFrame(() => { if (document.activeElement === trigger || document.activeElement === document.body) (saved ? button.current : trigger)?.focus(); }); }); }}>{t("reject")}</button></div>)}</div>}
...
export function Gallery({ i18n }: { i18n: HostI18n }) {
  ...
  return <section className="identityGallery" aria-label={t("title")}><style>{styles}</style>
    ...
    {gallery.error || photos.error ? <ErrorMessage t={t} onRetry={() => { pendingFocus.current = true; changed(); }} /> : (gallery.loading && !gallery.data) || (photos.loading && !photos.data) ? <p role="status">{t("loading")}</p> : <>
      ...
      {inspecting && <PhotoInspector observation={{ id: inspecting }} t={t} onClose={() => {
        setInspecting(null);
        requestAnimationFrame(() => {
          const button = photoButton.current;
          const collapsedGroup = button?.closest<HTMLDetailsElement>("details:not([open])");
          if (button?.isConnected && collapsedGroup) collapsedGroup.querySelector<HTMLElement>("summary")?.focus();
          else if (button?.isConnected) button.focus();
          else (profileHeading.current || gallerySearch.current)?.focus();
        });
      }} />}
      {(profile || review) && <><p className="identityMuted">{t("selection", { count: chosen.length })}</p><div className="identityGrid">{tiles.map((group, index) => { const item=group[0]; const identifiers=group.map((sample)=>sample.id); return <div key={item.id} className={`identityTile ${identifiers.every((id)=>selected.includes(id)) ? "isSelected" : ""}`}>
        ...
        <label><input type="checkbox" disabled={!writable || group.some((sample) => !sample.editable)} aria-label={review && group.length > 1 ? t("selectDisplayedGroup", { count: group.length, index: index + 1 }) : t("selectPhoto", { index: index + 1 })} ref={(element) => { if (element) element.indeterminate = identifiers.some((id) => selected.includes(id)) && !identifiers.every((id) => selected.includes(id)); }} checked={identifiers.every((id)=>selected.includes(id))} onChange={(event) => { const checked = event.target.checked; setSelected((previous) => checked ? Array.from(new Set([...previous,...identifiers])) : previous.filter((id) => !identifiers.includes(id))); }} /> {review ? t("visits",{count:new Set(group.map((sample)=>sample.occurrence_id)).size}) : item.reference ? t("referenceBadge") : item.confirmed ? t("identifiedBadge") : t("review")}</label>
        {review && group.length > 1 && <details className="identityGroupDetails">
          <summary>{t("reviewGroupPhotos", { count: group.length })}</summary>
          <p className="identityMuted">{t("groupSelectionHelp")}</p>
          <div className="identityGrid">{group.map((sample, photoIndex) => <div key={sample.id} className={`identityTile ${selected.includes(sample.id) ? "isSelected" : ""}`}>
            <button type="button" className="identityPhotoButton" aria-label={t("viewGroupPhoto", { index: photoIndex + 1, group: index + 1 })} onClick={(event) => { photoButton.current = event.currentTarget; setInspecting(sample.id); }}><Photo observation={sample} t={t} /></button>
            <label><input type="checkbox" disabled={!writable || !sample.editable} aria-label={t("selectGroupPhoto", { index: photoIndex + 1, group: index + 1 })} checked={selected.includes(sample.id)} onChange={(event) => { const checked = event.target.checked; setSelected((previous) => checked ? Array.from(new Set([...previous, sample.id])) : previous.filter((id) => id !== sample.id)); }} />{t("photoNumber", { index: photoIndex + 1 })}</label>
          </div>)}</div>
        </details>}
      </div>; })}</div>{!observations.length && <p>{t("emptyPhotos")}</p>}
