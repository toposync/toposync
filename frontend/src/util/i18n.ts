...
const translationsByLocale: Record<Locale, Translations> = {
  en: {
    ...
    "core.ui.pipelines.checks.unknown_operator": "Step {{step}} uses an unknown operator.",
    ...
    "core.ui.pipelines.alerts.tracking_unbounded_update_rate.suggestion": "Set default_interval_seconds to about 0.1-0.3, or add FPS Reducer/Throttle before heavy sinks such as Store Images, Notify, or Debug.",
    "core.ui.pipelines.alerts.notify_missing_store_images.message": "Notifications cannot display images because there is no Store Images step before Notify.",
    "core.ui.pipelines.alerts.notify_missing_store_images.suggestion": "Add Store Images before Notify so the main artifact has a stored reference.",
    "core.ui.pipelines.alerts.notify_thumbnail_not_stored.message": "Notify reads artifact '{{input_artifact_name}}', but upstream Store Images stores {{stored_artifacts}}.",
    "core.ui.pipelines.alerts.notify_thumbnail_not_stored.suggestion": "Store the same artifact that Notify reads, or set both steps to the same advanced artifact name.",
    "core.ui.pipelines.alerts.velocity_missing_camera_mapping.message": "Velocity estimation depends on world mapping, but there is no Camera Mapping step upstream.",
    "core.ui.pipelines.alerts.velocity_missing_camera_mapping.suggestion": "Add Camera Mapping before Velocity Estimation, or remove Velocity Estimation if you do not need it.",
    "core.ui.pipelines.alerts.velocity_after_rate_control.message": "Velocity Estimation runs after {{source_operator}}, which reduces update frequency and can make stopped_now/moving_now less responsive.",
    "core.ui.pipelines.alerts.velocity_after_rate_control.suggestion": "Place Velocity Estimation right after Camera Mapping and apply rate control later, for example before storage or notifications.",
    "core.ui.pipelines.alerts.area_missing_camera_mapping.message": "Area restriction depends on world mapping, but there is no Camera Mapping step upstream.",
    "core.ui.pipelines.alerts.area_missing_camera_mapping.suggestion": "Add Camera Mapping before Area Restriction, or set drop_when_unmapped=false if you want unmapped packets to pass through.",
    "core.ui.pipelines.alerts.debug_operator_enabled.message": "Debug step is enabled and may significantly reduce FPS or increase latency under load.",
    "core.ui.pipelines.alerts.debug_operator_enabled.suggestion": "Disable it once the pipeline is validated, or add throttling before debug or storage.",
    "core.ui.pipelines.alerts.store_images_without_rate_control.message": "Store Images is fed by object tracking without downstream rate control, which can be heavy on CPU or disk.",
    "core.ui.pipelines.alerts.store_images_without_rate_control.suggestion": "Add FPS Reducer/Throttle before Store Images to limit how many frames are stored per second.",
    "core.ui.pipelines.alerts.store_images_before_postprocess.message": "Store Images is placed before a post-processing step that may require artifact pixel data.",
    "core.ui.pipelines.alerts.store_images_before_postprocess.suggestion": "Move Store Images closer to the end of the pipeline, after image-processing and vision steps.",
    "core.ui.pipelines.alerts.store_images_missing_artifacts.message": "Store Images reads artifacts that are not produced upstream: {{missing_artifacts}}.",
    ...
    "core.ui.pipelines.alerts.camera_mapping_composition_missing.message": "Camera Mapping references composition '{{composition_id}}', but that composition does not exist.",
    ...
    "core.ui.pipelines.alerts.camera_mapping_camera_not_in_composition.scope.any": "in any composition available to Camera Mapping",
    ...
    "core.ui.pipelines.storage.subtitle": "Managed retention for files written by Store Images.",
  ...
  "pt-BR": {
    ...
    "core.ui.pipelines.checks.unknown_operator": "A etapa {{step}} usa um operador desconhecido.",
    ...
    "core.ui.pipelines.alerts.tracking_unbounded_update_rate.suggestion": "Defina default_interval_seconds em torno de 0.1-0.3, ou adicione Redutor de FPS/Throttle antes de saídas pesadas como Armazenar imagens, Enviar notificação ou Inspeção.",
    "core.ui.pipelines.alerts.notify_missing_store_images.message": "As notificações não conseguem mostrar imagens porque não há uma etapa Armazenar imagens antes de Enviar notificação.",
    "core.ui.pipelines.alerts.notify_missing_store_images.suggestion": "Adicione Armazenar imagens antes de Enviar notificação para que o artefato principal tenha uma referência armazenada.",
    "core.ui.pipelines.alerts.notify_thumbnail_not_stored.message": "Enviar notificação lê o artefato '{{input_artifact_name}}', mas Armazenar imagens nas etapas anteriores armazena {{stored_artifacts}}.",
    ...
    "core.ui.pipelines.alerts.velocity_missing_camera_mapping.message": "A estimativa de velocidade depende do mapeamento para o mundo, mas não há etapa Mapeamento para o mundo antes dela.",
    "core.ui.pipelines.alerts.velocity_missing_camera_mapping.suggestion": "Adicione Mapeamento para o mundo antes de Estimativa de velocidade, ou remova a estimativa se você não precisa dela.",
    "core.ui.pipelines.alerts.velocity_after_rate_control.message": "A Estimativa de velocidade roda depois de {{source_operator}}, o que reduz a frequência de atualização e pode deixar stopped_now/moving_now menos responsivo.",
    "core.ui.pipelines.alerts.velocity_after_rate_control.suggestion": "Posicione Estimativa de velocidade logo após Mapeamento para o mundo e aplique controle de frequência depois, por exemplo antes de armazenamento ou notificações.",
    "core.ui.pipelines.alerts.area_missing_camera_mapping.message": "A restrição de área depende do mapeamento para o mundo, mas não há etapa Mapeamento para o mundo antes dela.",
    "core.ui.pipelines.alerts.area_missing_camera_mapping.suggestion": "Adicione Mapeamento para o mundo antes de Restrição de área, ou defina drop_when_unmapped=false se quiser deixar pacotes sem mapeamento passarem.",
    "core.ui.pipelines.alerts.debug_operator_enabled.message": "A etapa de inspeção está ativada e pode reduzir bastante o FPS ou aumentar a latência sob carga.",
    "core.ui.pipelines.alerts.debug_operator_enabled.suggestion": "Desative depois de validar o fluxo, ou adicione controle de frequência antes de debug ou armazenamento.",
    "core.ui.pipelines.alerts.store_images_without_rate_control.message": "Armazenar imagens recebe dados do rastreamento sem controle de frequência depois dele, o que pode pesar em CPU ou disco.",
    "core.ui.pipelines.alerts.store_images_without_rate_control.suggestion": "Adicione Redutor de FPS/Throttle antes de Armazenar imagens para limitar quantos frames são armazenados por segundo.",
    "core.ui.pipelines.alerts.store_images_before_postprocess.message": "Armazenar imagens está antes de uma etapa de pós-processamento que pode precisar dos pixels do artefato.",
    "core.ui.pipelines.alerts.store_images_before_postprocess.suggestion": "Mova Armazenar imagens para mais perto do fim do fluxo, depois das etapas de imagem e visão.",
    "core.ui.pipelines.alerts.store_images_missing_artifacts.message": "Armazenar imagens lê artefatos que não são produzidos nas etapas anteriores: {{missing_artifacts}}.",
    ...
    "core.ui.pipelines.alerts.camera_mapping_composition_missing.message": "Mapeamento para o mundo referencia a composição '{{composition_id}}', mas essa composição não existe.",
    ...
    "core.ui.pipelines.alerts.camera_mapping_camera_not_in_composition.scope.any": "em nenhuma composição disponível para o Mapeamento para o mundo",
    ...
    "core.ui.pipelines.operator_description.core.schedule_gate": "Abre o fluxo apenas nos dias e horários escolhidos.",
    "core.ui.pipelines.operator_description.camera.motion_bgsub_adaptive":
    ...
    "core.ui.pipelines.editor.no_steps": "Nenhuma etapa ainda. Adicione operadores para montar o fluxo.",
    ...
    "core.ui.pipelines.storage.subtitle": "Retenção gerenciada para arquivos gravados por Store Images.",
