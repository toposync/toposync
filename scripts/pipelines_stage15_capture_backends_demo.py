...
class PrintSinkRuntime(SinkRuntime):
    ...
    async def process_packet(self, packet: Packet, context) -> list[Packet]:  # noqa: ANN001, ARG002
        ...
        frame_ts = (
            float(packet.payload.get("frame_ts", 0.0) or 0.0)
...
def _build_graph(args: argparse.Namespace) -> dict[str, Any]:
    ...
    nodes = [
        ...
        {
            "id": "sink",
            "operator": "demo.print_sink",
            "config": {"interval_seconds": float(args.print_interval_s)},
        },
    ]
    edges = [
        {
...
async def _run(args: argparse.Namespace) -> int:
    ...
    pipeline = Pipeline(name="stage15_capture_backends_demo", graph=graph)
...
def main() -> int:
    ...
    parser.add_argument(
        "--camera-id", default="", help="Camera ID from settings (optional if rtsp_url is set)"
    )
    ...
    parser.add_argument(
        ...
        help="Capture backend preference",
    )
    parser.add_argument("--fps", type=float, default=0.0, help="Optional override FPS (1..60)")
    parser.add_argument(
        "--poll-interval-ms", type=int, default=20, help="camera.source polling interval (1..250)"
    )
    parser.add_argument(
        "--print-interval-s", type=float, default=1.0, help="How often to print capture stats"
    )
