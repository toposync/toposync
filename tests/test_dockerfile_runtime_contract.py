...
def test_runtime_images_bundle_ptz_attention_extension() -> None:
    dockerfile = (ROOT / "Dockerfile").read_text(encoding="utf-8")

    assert dockerfile.count("uv build --wheel --out-dir /wheelhouse extensions/ptz_attention") == 1
    assert dockerfile.count("/wheelhouse/toposync_ext_ptz_attention-*.whl") == 2
